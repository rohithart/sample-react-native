...
const eventApi = {
  ...
  create: (orgId: string, data: Partial<Event>) => api.post<Event>(`/event/${orgId}`, data),
  update: (id: string, data: Partial<Event>) => api.put<Event>(`/event/${id}`, data),
  delete: (id: string) => api.delete<boolean>(`/event/${id}`),
