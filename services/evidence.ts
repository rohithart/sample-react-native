...
const evidenceApi = {
  ...
  create: (orgId: string, data: Partial<Evidence>) => api.post<Evidence>(`/evidence/${orgId}`, data),
  update: (id: string, data: Partial<Evidence>) => api.put<Evidence>(`/evidence/${id}`, data),
  delete: (id: string) => api.delete<boolean>(`/evidence/${id}`),
  getAllArchived: (orgId: string) => api.get<Evidence[]>(`/evidence/org/archived/${orgId}`),
  archive: (id: string) => api.patch<Evidence>(`/evidence/archive/${id}`, {}),
  unarchive: (id: string) => api.patch<Evidence>(`/evidence/unarchive/${id}`, {}),
  flag: (id: string, data: { reason: string }) => api.patch<Evidence>(`/evidence/flag/${id}`, data),
  unflag: (id: string) => api.patch<Evidence>(`/evidence/unflag/${id}`, {}),
  updateStatus: (id: string, data: { status: string }) => api.patch<Evidence>(`/evidence/status/${id}`, data),
