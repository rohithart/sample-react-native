...
export default function WorkflowDetailScreen() {
  ...
  const handleUnarchive = async () => {
    ...
    setIsLoading(false);
    setConfirmationType(null);
    Alert.alert('Success', 'Workflow unarchived successfully');
  };
  ...
  const actions: ActionItem[] = [
    ...
    { id: 'pdf', label: 'Download PDF', icon: <FileDown size={24} color={colors.success} />, onPress: () => downloadAndSharePdf('workflow', id || ''), color: 'success' as const },
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ConfirmationDialog
        ...
        isLoading={isLoading}
      />
      <EntityAttachments isVisible={showAttachments} onClose={() => setShowAttachments(false)} entity={'workflow'} entityId={id || ''} orgId={orgId || ''} />
      <EntityComments isVisible={showComments} onClose={() => setShowComments(false)} entity={'workflow'} entityId={id || ''} orgId={orgId || ''} />
      <EntityImages isVisible={showImages} onClose={() => setShowImages(false)} entity={'workflow'} entityId={id || ''} orgId={orgId || ''} />
      <EntityTimeline isVisible={showTimeline} onClose={() => setShowTimeline(false)} entity={'workflow'} entityId={id || ''} />
      <EntityHistory isVisible={showHistory} onClose={() => setShowHistory(false)} entity={'workflow'} entityId={id || ''} />
    </SafeAreaView>
  );
}
