...
export default function WorkflowDetailScreen() {
  ...
  const actions: ActionItem[] = [
    ...(isAdmin ? [{
      ...
      onPress: () => router.push(`/admin/workflow/${orgId}/${id}/edit`),
    ...
    { id: 'pdf', label: 'Download PDF', icon: <I.fileDown size={24} color={colors.success} />, onPress: () => downloadAndSharePdf(EntityType.WORKFLOW, id || ''), color: 'success' as const },
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <EntityAttachments isVisible={showAttachments} onClose={() => setShowAttachments(false)} entity={EntityType.WORKFLOW} entityId={id || ''} orgId={orgId || ''} />
      <EntityComments isVisible={showComments} onClose={() => setShowComments(false)} entity={EntityType.WORKFLOW} entityId={id || ''} orgId={orgId || ''} />
      <EntityImages isVisible={showImages} onClose={() => setShowImages(false)} entity={EntityType.WORKFLOW} entityId={id || ''} orgId={orgId || ''} />
      <EntityTimeline isVisible={showTimeline} onClose={() => setShowTimeline(false)} entity={EntityType.WORKFLOW} entityId={id || ''} />
      <EntityHistory isVisible={showHistory} onClose={() => setShowHistory(false)} entity={EntityType.WORKFLOW} entityId={id || ''} />
    </SafeAreaView>
  );
}
