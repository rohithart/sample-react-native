...
import { useFlagWorkorder, useUnflagWorkorder, useUpdateWorkorderStatus, useWorkorder } from '@/services/workorder';
...
export default function WorkOrderDetailScreen() {
  ...
  const flagFn = useFlagWorkorder(orgId || '');    
  const unflagFn = useUnflagWorkorder(orgId || '');
  ...
  const handleDelete = async () => {
    ...
    showToast({ type: 'success', title: 'Success', message: 'Work Order deleted successfully' });
  ...
  const handleArchive = async () => {
    ...
    setIsProcessing(false);
    setConfirmationType(null);
    showToast({ type: 'success', title: 'Success', message: 'Work Order archived successfully' });
  };
  ...
  const actions: ActionItem[] = [
    ...(isAdmin ? [{
      ...
      onPress: () => router.push(`/admin/workorder/${orgId}/${id}/edit`),
    ...
    { id: 'pdf', label: 'Download PDF', icon: <I.fileDown size={24} color={colors.success} />, onPress: () => downloadAndSharePdf(EntityType.WORKORDER, id || ''), color: 'success' as const },
        ...
        <View style={{ padding: 16, gap: 20 }}>
          {item.description && (
            <View style={{ backgroundColor: colors.card, padding: 16, borderRadius: 20, borderWidth: 1, borderColor: colors.border }}>
              <HtmlContent label="Work order Description" html={item.description} />
            </View>
          )}
