...
import { useEvidences } from '@/services/evidence';
...
export default function EvidencesListScreen() {
  ...
  const { data: items, isLoading, refetch, isRefetching } = useEvidences(id);
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ListItemCard
              ...
              status={item.status}
              onPress={() => router.push(`/admin/evidence/${id}/${item._id}`)}
            />
          )}
