...
export default function BookingTypesListScreen() {
  ...
  const { data: items, isLoading, refetch, isRefetching } = useBookingTypes(id);
  const refreshControl = useRefreshControl(refetch, isRefetching);

  const handleAdd = () => {
    router.push(`/admin/booking-type/new/${id}`);
  };

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      <Stack.Screen options={{ headerShown: false }} />
      <PageHeader icon="bookingType"
        title="Booking Types"
        rightAction={
          <Pressable
            onPress={handleAdd}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingTop: 60 }}>
              <Text style={{ color: colors.sub, fontSize: 15 }}>No booking types found</Text>
            </View>
          }
        />
      )}
