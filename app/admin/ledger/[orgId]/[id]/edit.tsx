...
export default function EditLedgerScreen() {
  ...
  const [name, setName] = useState('Sample Ledger');
  const [description, setDescription] = useState('This is a sample ledger description.');
  ...
  const handleSubmit = async () => {
    ...
    showToast({ type: 'success', title: 'Success', message: 'Ledger updated successfully' });
    router.push(`/admin/ledger/${orgId}/${id}`);
  };

  const handleDelete = () => {
    Alert.alert(
      'Delete Ledger',
      'This action cannot be reversed. The ledger will be permanently deleted.',
      [
