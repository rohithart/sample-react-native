...
export default function NewDocumentScreen() {
  ...
  const handleSubmit = async () => {
    ...
    showToast({ type: 'success', title: 'Document created successfully' });
    router.back();
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <PageHeader icon="document" title="New Document" />
      ...
      >
        <View style={{ backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: 8, padding: 12 }}>
          <Text style={{ fontSize: 12, color: colors.sub }}>Context: Folder: ${fId}</Text>
        </View>
