...
export default function NewEventFromTypeScreen() {
  ...
  const handleSubmit = async () => {
    ...
    showToast({ type: 'success', title: 'New Event created successfully' });
    router.back();
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <PageHeader icon="event" title="New Event" />
      ...
      >
        <View style={{ backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: 8, padding: 12 }}>
          <Text style={{ fontSize: 12, color: colors.sub }}>Context: Event Type: ${tId}</Text>
        </View>
